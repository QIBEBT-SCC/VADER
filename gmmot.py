...
def GaussianW2(mu0, mu1, S0, S1):
    """
    计算两个高斯分布之间的 Wasserstein 距离 (PyTorch版)
    mu: [d]
    S:  [d, d] 或 [d] (如果是对角阵)
    """
    ...
    if S0.dim() == 1:
        dist_sigma = torch.sum((torch.sqrt(S0 + 1e-7) - torch.sqrt(S1 + 1e-7))**2)
    else:
        S010 = torch.sqrt(torch.sqrt(S0) @ S1 @ torch.sqrt(S0) + 1e-6)
        dist_sigma = torch.trace(S0 + S1 - 2 * S010) 
...
def GW2(pi0,pi1,mu0,mu1,S0,S1):
    ...
    for k in range(K):
        dist_k = GaussianW2(mu0[k], mu1[k], S0[k], S1[k])
        M_list.append(dist_k)
    M = torch.stack(M_list)
    distGW2   = torch.sum(pi1*M)
    return distGW2
